...
class SimpleGraph(QMainWindow):
    def __init__(self):
        ...
        self.port = serial.Serial("COM19",self.baudrate)
        # UI Setup
        main_widget = QWidget()
        ...
        layout = QHBoxLayout(main_widget)
         ...
         # Menu boczne
        menu = QVBoxLayout()
        self.btns = {"Roll": self.roll, "Pitch": self.pitch, "Yaw": self.yaw}
        self.radios = {}
    ...
    def getVars(self):
        self.readSer()
        if len(self.data) == 16:
            self.czas = self.data[1]
            self.yaw = float(self.data[4])
            self.pitch = float(self.data[5])
            self.roll = float(self.data[6])

    def plot(self):
        self.ax.cla()
        for name,type in self.btns.items():
